...
- `当前状态`: `Fixing`
- `发现的问题`: 已知首轮回归面集中在 `client/gui` 路径。GUI 从旧的 packet/capability 流程切换到 payload/attachment 流程后，除了按钮交互、数据同步和命中框依赖关系变化外，还已知出现 missing text、dark/blurred entity preview，以及 severe readability degradation；在真正完成 GUI 手测和配套测试前，视为已发现的 GUI 回归待修项。
- `修复动作`: 修复重点放在 `MobDictionaryGui` 到 payload handler 的交互链、附件数据读取路径，以及与 `Box2D` 相关的点击区域校准；同时排查 missing text、dark/blurred entity preview 和 severe readability degradation 的渲染与文本布局原因，如发现旧事件类仍有残留假设，统一迁移到新的 payload/attachment 流程。
- `验证方式`: 运行 `./gradlew test --tests org.hhoa.mc.item_information.framework.Box2DTest --tests org.hhoa.mc.item_information.mobdictionary.network.PayloadCodecTest`，再进行 `MobDictionaryGui` 手动回归，确认文本不缺失、entity preview 不再 dark/blurred、可读性恢复且 GUI 打开、按钮点击、数据展示和命中框行为正常。
- `结论`: `client/gui` 条目按要求以 `Fixing` 落账；在 missing text、dark/blurred entity preview 和 severe readability degradation 全部消除，并且 GUI 回归和 payload/attachment 联调通过前，不接受该 diff。
