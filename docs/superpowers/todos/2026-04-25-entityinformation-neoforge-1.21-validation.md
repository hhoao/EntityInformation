...
- `当前状态`: `Fixing`
- `发现的问题`: 已知首轮回归面集中在 `client/gui` 路径。GUI 从旧的 packet/capability 流程切换到 payload/attachment 流程后，`MobDictionaryGui` 的文本曾走 detached matrix/batch draw path，导致 missing text，同时还伴随 dark/blurred entity preview 和 severe readability degradation。
- `修复动作`: 修复重点放在 `MobDictionaryGui` 到 payload handler 的交互链、附件数据读取路径，以及与 `Box2D` 相关的点击区域校准；文本部分已把 detached matrix path 替换为 `GuiGraphics.drawString`，并把锁定名称与进度文案抽到 `MobDictionaryTextContent`，剩余的 entity preview 仍需继续排查。
- `验证方式`: 运行 `./gradlew test --tests org.hhoa.mc.item_information.mobdictionary.client.gui.MobDictionaryTextContentTest --tests org.hhoa.mc.item_information.itemtooltip.ItemTooltipTest --console plain` 作为 Task 2 文本内容格式回归检查，并继续运行 `./gradlew test --tests org.hhoa.mc.item_information.framework.Box2DTest --tests org.hhoa.mc.item_information.mobdictionary.network.PayloadCodecTest`；实际 `TextRenderer`/`GuiGraphics`/真实 GUI 渲染路径仍需通过 `MobDictionaryGui` 手动回归确认文本是否可见且布局可读，entity preview 也继续按后续任务验证。
- `结论`: `client/gui` 条目按要求以 `Fixing` 落账；自动化测试已覆盖锁定名称与进度文本的内容格式，但真实 GUI 文本渲染路径仍待手动回归确认，entity preview 仍是未完成项，后续继续保留该条目直到整体 GUI 回归结束。
