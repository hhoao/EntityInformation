...
- `当前状态`: `Fixing`
- `发现的问题`: 已知首轮回归面集中在 `client/gui` 路径。GUI 从旧的 packet/capability 流程切换到 payload/attachment 流程后，`MobDictionaryGui` 的文本曾走 detached matrix/batch draw path，导致 missing text，同时 entity preview 仍需要从手工渲染迁回 1.21 的标准 inventory renderer。手动 smoke 进一步暴露出 dictionary 打开后整层 screen 被 blur 的回归，说明 GUI 仍在落回 1.21 `Screen` 的默认 backdrop/blur 路径。
- `修复动作`: 修复重点放在 `MobDictionaryGui` 到 payload handler 的交互链、附件数据读取路径，以及与 `Box2D` 相关的点击区域校准；文本部分已把 detached matrix path 替换为 `GuiGraphics.drawString`，并把锁定名称与进度文案抽到 `MobDictionaryTextContent`。本轮还把 entity preview 切到 `InventoryScreen.renderEntityInInventory`，并在 `MobDictionaryGui` 内显式接管 `renderBackground`、`renderBlurredBackground`、`renderTransparentBackground`，改为自绘半透明 backdrop，避免整层 UI 落回默认 blur。
- `验证方式`: 运行 `./gradlew test --tests org.hhoa.mc.item_information.mobdictionary.client.gui.MobDictionaryTextContentTest --tests org.hhoa.mc.item_information.mobdictionary.client.gui.MobDictionaryEntityPreviewStateTest --tests org.hhoa.mc.item_information.mobdictionary.client.gui.MobDictionaryGuiBackgroundTest --console plain` 作为 GUI 文本内容、preview state 与 backdrop hook 回归检查，并继续运行 `./gradlew test --tests org.hhoa.mc.item_information.framework.Box2DTest --tests org.hhoa.mc.item_information.mobdictionary.network.PayloadCodecTest --console plain`；blur/backdrop 的最终可视效果仍需通过 `MobDictionaryGui` 手动回归确认。
- `结论`: `client/gui` 条目按要求继续以 `Fixing` 落账；文本路径、entity preview 和 backdrop hook 都已补丁化，但真实 GUI 手动回归仍待确认，所以该条目继续保留为未完全验收状态。
